...
struct gatt_db_service {
    ...
    // general notification callback for read and write
    gatt_db_read_t read_notify;
    gatt_db_write_t write_notify;
};
                             ...
                             gatt_db_write_t write_notify)
{
    ...
    // for user callback notification
    service->read_notify = read_notify;
    service->write_notify = write_notify;
...
struct gatt_db_attribute *gatt_db_add_service(struct gatt_db *db,
                        ...
                        uint16_t num_handles)
{
    return gatt_db_insert_service(db, db->next_handle, uuid, primary, num_handles, NULL, NULL);
}
struct gatt_db_attribute *gatt_db_add_service_ext(struct gatt_db *db,
                        const bt_uuid_t *uuid,
                        bool primary,
                        uint16_t num_handles,
                        gatt_db_read_t read_notify,
                        gatt_db_write_t write_notify)
{
    return gatt_db_insert_service(db, db->next_handle, uuid, primary, num_handles, read_notify, write_notify);
}
                ...
                gatt_db_attribute_read_t func, void *user_data)
{
    ...
    if (attrib->service->read_notify) {
        attrib->service->read_notify(attrib, attrib->handle, offset, opcode, att, attrib->user_data);
    }
                    ...
                    void *user_data)
{
    ...
    if (attrib->service->write_notify) {
        attrib->service->write_notify(attrib, attrib->handle, offset, value, len, opcode, att, attrib->user_data);
    }
