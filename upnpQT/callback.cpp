...
int default_handling(Upnp_EventType EventType, void *Event, void *Cookie)
{
    ...
    case UPNP_DISCOVERY_SEARCH_RESULT:
    {
        ...
        {
            SampleUtil_Print("Error in Discovery Callback -- %d", d_event->ErrCode);
        }

        if ((ret = UpnpDownloadXmlDoc(d_event->Location, &DescDoc)) != UPNP_E_SUCCESS)
        {
            SampleUtil_Print("Error obtaining device description from %s -- error = %d", d_event->Location, ret);
        }
        else
        {
    ...
    case UPNP_DISCOVERY_ADVERTISEMENT_BYEBYE:
    {
        ...
        if (d_event->ErrCode != UPNP_E_SUCCESS)
        {
            SampleUtil_Print("Error in Discovery ByeBye Callback -- %d", d_event->ErrCode);
        }
    ...
    case UPNP_CONTROL_GET_VAR_COMPLETE:
    {
        ...
        {
            SampleUtil_Print("Error in Get Var Complete Callback -- %d", sv_event->ErrCode);
        }
        else
        {
            TvCtrlPointHandleGetVar(sv_event->CtrlUrl, sv_event->StateVarName, sv_event->CurrentVal);
        }
    ...
    case UPNP_EVENT_RENEWAL_COMPLETE:
    {
        ...
        {
            SampleUtil_Print("Error in Event Subscribe Callback -- %d", es_event->ErrCode);
        }
        else
        {
